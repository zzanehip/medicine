...
void untar_create_link_(char * fullpath, char * linkpath, int filesize, FILE* fp){
  ...
  if(result != 0) {
    fprintf(stdout, "Could not create symlink %s (%d)\n", fullpath, result);
...
void untar_create_directory_(char * fullpath, mode_t mode, int filesize, FILE* fp){
  ...
  if(result != 0) {
    //if(!errno == EEXIST)
    //  fprintf(stdout, "Could not create directory %s (%d)\n", fullpath, result);
...
void untar_create_file_(char * fullpath, mode_t mode, int filesize, FILE*fp){
  ...
  if(!out) {
    fprintf(stdout, "Could not create file %s\n", fullpath);
    ...
    for(;;) {
      ...
      switch(header.linkindicator) {
        ...
        case '5':
          ...
          break;
        default:
          fprintf(stdout, "Unsupported file type %c; skipping.\n", header.linkindicator);
      }
